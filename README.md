...
# Humanize KR — 한국어 문체 품질 개선기 v2.1.0-internal
...
> **내부 fork 운영 주의:** 우리 Hermes multi-profile 시스템에는 `install.sh`/`update.sh` 전역 심링크 설치를 바로 적용하지 않습니다. 실제 적용은 별도 Hermes profile-local skill wrapper에서 검증 후 단계적으로 진행합니다. 아래 설치 방법은 upstream 공개 사용자를 위한 참고입니다.

> **Claude Code**와 **OpenAI Codex CLI** 양쪽을 지원합니다. 전체 가이드: [`INSTALL.md`](INSTALL.md)

**Claude Code — 플러그인 마켓플레이스 (클론 불필요, upstream 공개 설치 기준)**
...
| C | 구조적 AI 패턴 | 기계적 "첫째/둘째/셋째", 과도한 불릿·헤딩·이모지, 연결어미 뒤 쉼표 (C-11), **가운데점(·) 결합도 무시 나열 (C-13)** |
...
- 본 내부 fork는 **AI 탐지기 우회(Undetectable AI)**가 아니라 **한국어 글쓰기 품질 개선·번역투 제거·가독성 향상**을 목적으로 합니다.

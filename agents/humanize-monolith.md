...
- 변경률 50% 초과 도달: 마지막 안전 버전으로 롤백 후 출력. final.md의 `<!-- HUMANIZE-SUMMARY -->` 블록에 `over_polish_aborted: true` 기록.
- 자체검증 항목 위반 후 1회 재시도에도 미해결: 결과 출력 + final.md의 `<!-- HUMANIZE-SUMMARY -->` 블록에 위반 항목 명시.

# Quick Rules — Monolith Fast Path 전용 (v2.1)
...
| C-13 | 가운데점(·) 결합도 무시 단순 나열 | S2 | 본문 산문에서 결합도 낮은 독립 항목이 반복될 때만 쉼표·조사·문장 분리로 풀어쓴다. 보존: 한·일 관계, 상·하반기, 금·은·동메달, 고유표기, 제목/표/태그/UI 라벨/짧은 레이블, 하위 그룹화 |
...
위반 시: edit 롤백 → 다시 윤문 → 재점검. 자체 루프 최대 1회. 이상 미해결이면 결과를 그대로 출력하되 `final.md` 끝 HTML 주석 블록에 "자가검증 미통과 항목 N건" 표기.
...
> v2.1 신규는 C-13 가운데점(·) 결합도 무시 나열 [S2] **1건** — 국립국어원 문장부호 규정 기반(기본 쉼표, 짝·밀접 묶음만 가운데점). 룰 전용 트랙(정량 metric 미부여).

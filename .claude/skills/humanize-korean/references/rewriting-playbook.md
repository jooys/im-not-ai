...
- **가운데점(·) 결합도 무시 나열 (C-13)**: 기본은 쉼표.
  - 결합도 약한 단순 나열 → 쉼표: "정치·경제·사회·문화 전반" → "정치, 경제, 사회, 문화 전반".
  - 짝·밀접 묶음은 가운데점 보존: "한·일 관계", "상·하반기", "금·은·동메달", "논문·보고서".
  - 하위 그룹화가 있는 복합 나열은 혼용 유지: "철수·영희, 민수·수진 팀"(작은 묶음=·, 큰 분류=쉼표).
...
- **가운데점(·)**: 부호 남용 같은 결 → C 절 처방 참조(C-13). 단순 나열은 쉼표, 짝·밀접 묶음만 가운데점.

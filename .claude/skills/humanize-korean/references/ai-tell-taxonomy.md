...
> **v2.1 추가 (2026-06-19):** `C-13` 가운데점(·) 결합도 무시 나열 [S2] 신설. 국립국어원 문장부호 규정(문화체육관광부 고시 제2014-42호) 기반 — 쉼표=독립 항목 단순 나열, 가운데점=짝·밀접 묶음. AI가 결합도와 무관하게 가운데점을 기본 나열 부호로 남발하는 패턴을 잡아 쉼표로 환원하되, 의미상 한 쌍·하위 묶음(`한·일`·`상·하반기`·`금·은·동`)은 보존. 룰 전용 트랙(metric 미부여 — 결합도 판단은 LLM 몫). **A-17 carry-forward**: A-17 '-들' 부착은 NMT 원본 출력 회차가 아직 없어 본 v2.1에서도 hold 유지, 이후 회차에서 동일 ID로 재평가(아래 A-17 hold 기록 참조).
>
...
### C-13. 가운데점(·) 결합도 무시 나열 [S2] · v2.1 신규
- 패턴: 결합도(밀접성)가 약한 단순 나열에 쉼표 대신 가운데점(·, 중점/가운뎃점)을 기본 부호로 사용. AI는 영어 slash/and 나열·압축 감각으로 항목 간 결합도와 무관하게 가운데점을 디폴트로 박는다. C-11/C-12(쉼표 위치·밀도)와 측정 차원이 다르고, J(시각 장식)의 대시(—) 남용 규칙과 같은 결의 부호 남용. 근거: 국립국어원 문장부호 규정(문화체육관광부 고시 제2014-42호) — 쉼표는 같은 자격의 어구를 독립적으로 단순 나열할 때, 가운데점은 짝을 이루거나 밀접한 관계로 묶여 하나의 단위처럼 쓰일 때.
- 적용 가드: **본문 산문에서 결합도 낮은 독립 항목이 반복될 때만 후보로 표시한다.** 제목, 표, 목차, 태그, UI 라벨, 명령·파일 경로·코드 식별자, 기관·제품·프로젝트의 고유 표기, 짧은 압축 레이블, 파이프라인 단계명, 인용문 안 표기는 자동 수정하지 않는다. 애매하면 쉼표로 바꾸지 말고 HOLD 또는 사용자 확인으로 넘긴다.
- 예(남용): "**정치·경제·사회·문화** 전반에 영향" / "**탐지·분류·재작성** 단계" — 독립 항목을 가운데점으로 단순 열거.
- 비대상(보존): 짝·밀접 묶음·하위 그룹화 — "**한·일** 관계", "**상·하반기**", "**금·은·동**메달", "**논문·보고서** 형태", "**노사·정부**", 기관명·고유표기, 복합 나열의 작은 묶음("**철수·영희**, **민수·수진** 팀"). 2014 개정으로 가운데점 자리에 쉼표도 허용되나, 의미 구별·가독성을 위해 두 부호 구분은 유지한다.
- 처방: (a) 결합도 약한 나열은 쉼표로 환원("정치·경제·사회·문화" → "정치, 경제, 사회, 문화"), (b) 또는 "과/와·및·하고" 연결조사로, 일부는 문장 분리, (c) 하위 그룹화가 필요한 복합 나열은 가운데점(작은 묶음) + 쉼표(큰 분류) 혼용을 유지. 한 문서에서 결합도 약한 가운데점 나열이 반복될수록 강도 가산.
- Do-NOT: 본 분류 문서·표·인용·고유표기 자체의 표기 관용 가운데점은 윤문 대상이 아니다. 적용 범위는 **본문 산문 내 단순 나열**에 한정.
